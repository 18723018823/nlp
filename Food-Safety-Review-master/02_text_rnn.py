...
def create_text_rnn():
    ...
    l_lstm = CuDNNLSTM(128)(embedding_input)
    output = Dense(2, activation='softmax')(l_lstm)
...
for i, (train_index, valid_index) in enumerate(skf.split(x_train, train['label'])):
    ...
    model.compile(loss='categorical_crossentropy',
                  ...
                  metrics=['acc'])
    model.summary()
    checkpoint = ModelCheckpoint(filepath='models/rnn_text_{}.h5'.format(i + 1),
                                 monitor='val_loss',
...
sub['label'] = labels
sub.to_csv('result/rnn.csv', index=None)
